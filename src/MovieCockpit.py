...
class MovieCockpitSummary(ScreenSummary):
...
class MovieCockpit(Screen, HelpableScreen, CockpitContextMenu, Actions, CutList):
    ...
    def __init__(self, session, infobar_instance):
        ...
        if os.path.basename(self.return_dir) == ".Trash":
        ...
        if current_dir:
            ...
            for bookmark in bookmarks:
                if current_dir.startswith(bookmark):
                    ...
                    if title_dir.startswith(".Trash"):
                        title_dir = title_dir[len(".Trash") + 1:]
                    break
            if title_dir:
                title += " - " + title_dir
        self.setTitle(title)
    ...
    def openSetupScreen(self):
        self.session.openWithCallback(
            self.openSetupScreenCallback, SetupScreen)

    def openSetupScreenCallback(self, reload_moviecockpit=False):
    ...
    def deleteMovies(self):
        ...
        for path in selection_list:
            logger.debug("path: %s", path)
            if not path.endswith(".Trash") and not path.endswith(".."):
    ...
    def openTrashcan(self):
        self.changeDir(MountCockpit.getInstance().getHomeDir(ID) + "/.Trash")
